...
class BasicTable extends PureComponent {
  ...
  getData = () => {
    let _this = this;
    request.post(apis.getDataSource, {
  ...
  onRowClick = (record, index) => { //record 这一行数据  index:索引
    let selectKey = [index]; // 获取selectKey 固定格式  获取index 索引  这为什么使用数组呢  因为他是个单选  单选理论上就是一个值  多选话就是一个数组了，这里主要是考虑格式规范 
    Modal.info({
      title: '信息',
      content: `用户名：${record.userName},用户爱好：${record.interest}`
    })
    this.setState({ // 点击记住两个字段  第一个是 我们选中的是哪一个索引  第二个  就是我们选中的是哪一个项 （包含了这一行的数据 ） 我们就可以知道我们在提交  修改 删除的时候  我们就能够拿到用户id 了，进行绑定和删除
  ...
  render() {
    ...
    const rowSelection = { // 单选按钮 点击这一行要把当前这一行的信息存储下来 而且需要获取当前用户的id 或者说获取到当前用户的姓名   
      type: 'radio',
    ...
    const rowCheckSelection = {
      ...
      onChange: (selectedRowKeys, selectedRows) => { // 作用 ：每一个checkbo框上面添加事件 上下变化了以后去给他一个记录
      ...
      <div>
        <Card title="基础表格">
          <Table
            ...
            pagination={false} // 设置是否要分页 false  不需要分页 
          />
        </Card>
        ...
        <Card title="Mock-单选" style={{ margin: '10px 0' }}>
          <Table
            ...
            onRow={(record, index) => { // onRow  设置行 （选中某一行去选中单选按钮）通过onRow  设置 控制我们去点击某一行 
              return {
                onClick: () => {
