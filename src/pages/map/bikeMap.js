...
export default class Order extends React.Component {
  ...
  state = { // res.result.total_count 使用  就需要定义state,否则就会报错
    bikeInfo: {}
  }
  ...
  requestList = () => {
    axios.ajax({
      url: '/map/bike_list',
      data: {
        params: this.params // this.params  属于当前组件的全局对象
      ...
      if (res) {
        this.setState({
          ...
          //res.result.total_count  这个值是在render中需要使用到的，必须要把他存下来，在render中 才能取到。
        }, () => {

        })
        this.renderMap(res.result); // 根据查询到的结果数据进行渲染地图，接收渲染地图的结果，根据结果去渲染，去处理我们的数据
        // res.result  就不需要往state里面去存了，因为他不需要去渲染render
        // 只需要操作这个div container,通过地图服务的能力 百度地图SDK  操作地图就可以了
      }
    })
  }

  // 查询表单  // handleFilterSubmit接收整个查询功能返回的参数   在参数里面去实现我们的查询功能
  ...
  renderMap = (res) => {
    ...
    // 路线列表 保存路线的坐标点
    let routeList = [];
    list.forEach((item) => { // item 里面的坐标点是一个字符串
    ...
    bikeList.forEach((item) => { // 遍历自行车的坐标点
      let p = item.split(",");
      // 通过这种方式创建坐标点
      let point = new window.BMap.Point(p[0], p[1]);
