...
export default class Bar extends React.Component {
  ...
  getOption() {
    let option = { // 配置 肯定是一个object  // 如何定义
      ...
      yAxis: { // y 轴 主要是呈现的我们的数据量  数据的一个值，不需要配备很多东西  因为他这个值会自动的去计算 ，指定type 指向value
      ...
      series: [
        // 我们只需要定义数据源
        {
          name: '订单量',
          type: 'bar', // 代表柱形图
          data: [ // 只需要写7个就可以了 ，和x 轴的data 保持一一对应 千万不能说少一个 多一个   否则会出问题
  ...
  getOption2() {
    let option = {
      ...
      series: [ // 数组形式 写三个 就可以把三个的订单量加载出来
        // 定义series  加载我们的数据源
        {
          name: 'OFO',
          type: 'bar',
